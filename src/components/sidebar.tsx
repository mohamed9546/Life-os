...
import {
  ...
  ChevronRight, Zap, Heart, Workflow, ShieldCheck, X,
} from "lucide-react";
...
export function Sidebar({ user }: { user: AuthenticatedAppUser }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("open-mobile-nav", openMobileNav as EventListener);
    window.addEventListener("close-mobile-nav", closeMobileNav as EventListener);
    window.addEventListener("resize", handleResize);
    window.addEventListener("keydown", handleKeydown);
    return () => {
      window.removeEventListener("open-mobile-nav", openMobileNav as EventListener);
      window.removeEventListener("close-mobile-nav", closeMobileNav as EventListener);
  ...
  useEffect(() => {
    document.documentElement.setAttribute("data-mobile-nav-open", String(mobileOpen));
      ...
      >
        ...
        >
          <div className={cn("flex items-center", collapsed ? "justify-center" : "gap-3")}>
            ...
            <button
              onClick={() => setMobileOpen(false)}
              className="btn-icon btn-ghost ml-auto text-slate-400 hover:bg-white/6 hover:text-white lg:hidden"
              aria-label="Close navigation"
            >
              <X size={16} />
            </button>
