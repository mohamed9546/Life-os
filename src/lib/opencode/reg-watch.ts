import { writeOpenCodeJson, writeOpenCodeText, readOpenCodeJson } from "./storage";
...
export async function buildRegWatchDigest() {
  const today = new Date().toISOString().slice(0, 10);
  const seen = await readOpenCodeJson<Record<string, string>>("cache/reg-watch-seen.json", {});
  const fresh: Array<{ feed: string; title?: string; link?: string; published?: string; summary?: string; error?: string }> = [];
  ...
  await writeOpenCodeJson("cache/reg-watch-seen.json", seen);
  ...
  return { generatedAt: today, items: fresh };
}
