...
import { opencodePath, saveOpenCodeText, slugify } from "./lib.mjs";

const rl = readline.createInterface({ input, output });
const identifier = (process.argv[2] || await rl.question("PMID or DOI: ")).trim();
await rl.close();

if (!identifier) {
  throw new Error("A PMID or DOI is required.");
}

let note;
if (/^\d+$/.test(identifier)) {
  note = await fetchPubMedNote(identifier);
} else {
  note = await fetchCrossrefNote(identifier);
}

const fileName = `${note.idType}-${slugify(note.id)}-${slugify(note.title || "paper")}.md`;
await saveOpenCodeText(`notes/papers/${fileName}`, renderNote(note));
console.log(`Saved ${fileName}`);

async function fetchPubMedNote(pmid) {
...
async function fetchCrossrefNote(doi) {
  ...
  return {
    ...
    year: message.issued?.["date-parts"]?.[0]?.[0] || "Unknown year",
